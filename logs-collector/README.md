# Kubernetes Logs Collector

Maintained by **Mattox Engineering**
...
* Save the script as: `rancher2_logs_collector.sh`
  ...
  Using `wget`:
    ```bash
    wget --backups https://raw.githubusercontent.com/supporttools/scripts/main/logs-collector/rancher2_logs_collector.sh
    ```
  Using `curl`:
    ```bash
...
### Optional: Download and run the script in one command
  ```bash
  curl -Ls https://raw.githubusercontent.com/supporttools/scripts/main/logs-collector/rancher2_logs_collector.sh | sudo bash
  ```
...
```
Kubernetes logs-collector (Mattox Engineering)
